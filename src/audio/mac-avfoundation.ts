  ...
  async start(opts: CaptureOptions): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      proc.on('error', (err) => {
        ...
        reject(new AudioCaptureError(`Failed to start ffmpeg: ${err.message}. Grant microphone access in System Settings → Privacy & Security → Microphone.`));
      });
    });
  }
...
export class MacAvFoundationFactory implements AudioCaptureFactory {
  create(): AudioCapture { return new MacAvFoundationCapture(); }
  ...
  async listSources(): Promise<AudioSource[]> {
    try {
      ...
      for (const line of stderr.split('\n')) {
        ...
        if (match) {
          sources.push({ id: match[1], label: match[2], isDefault: match[1] === '0' });
        }
      }
  ...
  defaultSource(): string { return process.env.RECMP3_SOURCE ?? '0'; }
}
