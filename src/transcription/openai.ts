...
export class OpenAIProvider implements TranscriptionProvider {
  ...
  async transcribe(input: TranscriptionInput): Promise<TranscriptionResult> {
    ...
    log.info(`Transcribing with OpenAI (${this.config.model}): ${basename(audioPath)}`);
    ...
    try {
      ...
      throw new NetworkError(`Network error connecting to OpenAI: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      clearTimeout(timeout);
    }
    ...
    if (!response.ok) {
      ...
      throw new TranscriptionError(`OpenAI API error ${response.status}: ${body || response.statusText}`, response.status);
    }

    const raw = await response.json() as Record<string, unknown>;
    return {
      text: (typeof raw === 'string' ? raw : ((raw.text as string) ?? '')).trim(),
      ...
      segments: (raw.segments as Array<{ start: number; end: number; text: string }> | undefined)?.map((s) => ({
        startSec: s.start, endSec: s.end, text: s.text,
      })),
      raw,
      provider: 'openai',
